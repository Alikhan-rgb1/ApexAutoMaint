      ...
      <div className="flex items-end justify-between gap-6">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="bg-dark border border-white/10 rounded-lg px-4 py-3 text-white text-xs font-bold uppercase tracking-widest focus:outline-none focus:border-gold"
          >
            <option value="all" className="bg-dark">{t.portal.orders.all}</option>
      ...
      <div className="bg-dark-lighter border border-white/10 rounded-2xl overflow-hidden">
          ...
          <table className="min-w-[860px] w-full">
            <thead className="bg-dark border-b border-white/10">
              <tr className="text-left text-xs font-bold uppercase tracking-widest text-gray-400">
                <th className="px-6 py-4">{t.portal.orders.date}</th>
                <th className="px-6 py-4">{t.portal.orders.vehicle}</th>
                <th className="px-6 py-4">{t.portal.orders.services}</th>
                <th className="px-6 py-4">{t.portal.orders.status}</th>
                <th className="px-6 py-4 text-right">{t.portal.orders.total}</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                rows.map((o) => (
                  <tr key={o.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="px-6 py-5 text-sm text-gray-200">
                      <Link href={`/portal/orders/${o.id}`} className="hover:text-gold transition-colors">
                        {formatDate(o.serviceDate ?? o.createdAt, locale)}
                      </Link>
                    </td>
                    <td className="px-6 py-5 text-sm text-gray-200">
                      {o.vehicle ? `${o.vehicle.make} ${o.vehicle.model} (${o.vehicle.year})` : '-'}
                    </td>
                    <td className="px-6 py-5 text-sm text-gray-400">
                      {(o.items ?? []).slice(0, 2).map((i) => i.serviceType).join(', ') || '-'}
                    </td>
        ...
        <div className="flex items-center justify-between px-6 py-4">
          <div className="text-xs text-gray-400">
            {t.portal.orders.pageOf.replace('{page}', String(page)).replace('{total}', String(totalPages))}
          </div>
